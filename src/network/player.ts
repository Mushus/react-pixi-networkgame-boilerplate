...
import ServerConnection, { ConnectionEvent, ResponseRequestP2P, ResponseResponseP2P } from '@/network/server';
...
export default class PlayerConnection {
  ...
  createPeer(initiator: boolean) {
    ...
    if (!initiator) {
      console.log(initiator)
      this.serverConnection.once(ConnectionEvent.RequestP2P, (data: ResponseRequestP2P) => {
        console.log(data)
      })
    }
  }
  ...
  request(userId: string) {
    return new Promise(resolve => {
      ...
      this.serverConnection.once(ConnectionEvent.ResponseP2P, (data: ResponseResponseP2P) => {
